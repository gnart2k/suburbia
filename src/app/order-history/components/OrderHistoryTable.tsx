                ...
                {orders.length > 0 ? (
                    orders.map((order, index) => (
                        <TableRow key={order.id}>
                            ...
                            <TableCell className="border-r border-gray-200 text-center">
                                <span className={cn("text-white px-4 py-2 rounded-md",
                                    order.status == OrderStatus.Processing && "bg-yellow-200 text-yellow-800",
                                    order.status == OrderStatus.Paid && "bg-blue-200 text-blue-800",
                                    order.status == OrderStatus.Delivered && "bg-blue-200 text-blue-800",
                                    order.status == OrderStatus.Completed && "bg-green-200 text-green-800",
                                    order.status == OrderStatus.Canceled && "bg-red-200 text-red-800",
                                )}>
                                {order.status}
                                </span>
                            </TableCell>
